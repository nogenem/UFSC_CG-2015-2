...
class Coordinate
{
    public:
        Coordinate(){}
        Coordinate(double cx, double cy, double cz = 0.0) :
            x(cx), y(cy), z(cz), w(1.0){}
        virtual ~Coordinate(){};
...
class Curve : public Object
{
    ...
    protected:
            ...
            float m_step = 0.02; //Passo usado na criação da curva
};
...
class Surface : public Object
{
    ...
    protected:
            ...
            Coordinates m_controlPoints;
            float m_step = 0.02; //Passo usado na criação da superficie

            int m_maxLines = 4, m_maxCols = 4;// Numero de linhas e colunas da matriz da surperficie
...
// Algoritmos baseados na implementação do professor
class BSplineSurface : public Surface
{
    ...
    private:
        ...
        // Matrizes usadas na criação da superficie
		Matrix4x4 m_coordsX, m_coordsY, m_coordsZ;
		Matrix4x4 m_cx, m_cy, m_cz;
		Matrix4x4 m_es, m_et;
		Matrix4x4 m_DDx, m_DDy, m_DDz;

		static Transformation m_M;// Matriz do método B-Spline
        static Transformation m_Mt;// Matriz do método B-Spline transposta

    private:
		void updateCoordsMatrices(int nLine, int nCol);
		void updateForwardDiffMatrices();
		void calculateCoefficients();
		void createDeltaMatrices();
		void createForwardDiffMatrices();
		void createCurve(double delta,
